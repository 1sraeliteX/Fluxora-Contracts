...
fn test_withdraw_multiple_partial_withdrawals() {
    ...
    assert_eq!(amt1, 200);
    assert_eq!(ctx.client().get_stream_state(&stream_id).withdrawn_amount, 200);

    // t=500: withdraw 300 (500 - 200)
    ctx.env.ledger().set_timestamp(500);
    let amt2 = ctx.client().withdraw(&stream_id);
    assert_eq!(amt2, 300);
    assert_eq!(ctx.client().get_stream_state(&stream_id).withdrawn_amount, 500);

    // t=900: withdraw 400 (900 - 500)
    ctx.env.ledger().set_timestamp(900);
    let amt3 = ctx.client().withdraw(&stream_id);
    assert_eq!(amt3, 400);
    assert_eq!(ctx.client().get_stream_state(&stream_id).withdrawn_amount, 900);
...
fn test_withdraw_cap_contract_balance_safety() {
    ...
    let amt = ctx.client().withdraw(&stream_id);
    assert_eq!(amt, 600, "Withdrawn amount must be capped by contract balance");
    
    let state = ctx.client().get_stream_state(&stream_id);
    assert_eq!(state.withdrawn_amount, 600);
    assert_eq!(state.status, StreamStatus::Active, "Must not be completed if capped by balance");
...
fn test_batch_withdraw_running_balance_cap() {
    ...
    let id1 = ctx.client().create_stream(&ctx.sender, &ctx.recipient, &500, &1, &0, &0, &500);
    let id2 = ctx.client().create_stream(&ctx.sender, &ctx.recipient, &500, &1, &0, &0, &500);
    ...
    assert_eq!(ctx.client().get_stream_state(&id1).status, StreamStatus::Completed);
    assert_eq!(ctx.client().get_stream_state(&id2).status, StreamStatus::Active);
}
